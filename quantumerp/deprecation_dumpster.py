...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the Qunatum ERP app. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The Qunatum ERP version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
def get_pos_profile_item_details(company, ctx, pos_profile=None, update_data=False):
	from quantumerp.stock.get_item_details import get_pos_profile_item_details_
...
def get_item_warehouse(item, ctx, overwrite_warehouse, defaults=None):
	from quantumerp.stock.get_item_details import get_item_warehouse_
