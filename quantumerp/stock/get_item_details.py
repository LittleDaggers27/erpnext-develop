...
import quantumerp
from quantumerp import get_company_currency
from quantumerp.accounts.doctype.pricing_rule.pricing_rule import (
...
from quantumerp.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from quantumerp.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from quantumerp.stock.doctype.price_list.price_list import get_price_list_details
...
@frappe.whitelist()
@quantumerp.normalize_ctx_input(ItemDetailsCtx)
...
def update_stock(ctx, out, doc=None):
	from quantumerp.stock.doctype.batch.batch import get_available_batches
	from quantumerp.stock.doctype.serial_no.serial_no import get_serial_nos_for_outward
...
def validate_item_details(ctx: ItemDetailsCtx, item):
	...
	from quantumerp.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(ctx: ItemDetailsCtx, item, overwrite_warehouse=True) -> ItemDetails:
	...
	if item.is_fixed_asset:
		from quantumerp.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
	...
	if ctx.doctype in purchase_doctypes and not frappe.db.get_single_value(
		...
		from quantumerp.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
...
from quantumerp.deprecation_dumpster import get_item_warehouse
...
def get_price_list_rate(ctx: ItemDetailsCtx, item_doc, out: ItemDetails = None):
	...
	if meta.get_field("currency") or ctx.get("currency"):
		...
		if not ctx.is_internal_supplier and not out.price_list_rate and ctx.transaction_type == "buying":
			from quantumerp.stock.doctype.item.item import get_last_purchase_details
...
def validate_conversion_rate(ctx: ItemDetailsCtx, meta):
	from quantumerp.controllers.accounts_controller import validate_conversion_rate
...
from quantumerp.deprecation_dumpster import get_pos_profile_item_details
...
def get_bin_details(item_code, warehouse, company=None, include_child_warehouses=False):
	...
	if warehouse:
		...
		from quantumerp.stock.doctype.warehouse.warehouse import get_child_warehouses
...
def get_batch_qty(batch_no, warehouse, item_code):
	from quantumerp.stock.doctype.batch import batch
